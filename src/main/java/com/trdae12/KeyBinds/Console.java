...
public class Console extends GuiScreen
{
  ...
  public static HashMap<String, String> help = new HashMap<String, String>();
    ...
    public void handleInput(String in){
    	if(in.toLowerCase().startsWith("bind")){
    		if(!in.toLowerCase().contains(" ")){
    			Minecraft.getMinecraft().thePlayer.sendChatMessage(EnumChatFormatting.RED+"Usage: bind <key> <command/message>");
    			return;
    		}
    		...
    		if(args.length > 2){
	    		...
	    		KeyBindsMod.log(keyToBind+" = "+Keyboard.getKeyIndex(keyToBind.toUpperCase()));
	    		...
	    		for(KeyBinding bind : binds){
		    		if(Keyboard.getKeyIndex(keyToBind.toUpperCase()) == bind.getKeyCode()){
		    			...
		    			if(loc.length() == 0) loc = bind.getKeyDescription();
		    			if(loc.contains(".")){
		    				loc = loc.split("\\.")[1];
		    			}
		    			Minecraft.getMinecraft().thePlayer.sendChatMessage("Not binding over the "+loc+" key!");
		    			return;
	
		    		}
	    		}
	    		if(Keyboard.getKeyIndex(keyToBind.toUpperCase()) > 0){
		    		KeyBindsMod.log("Binding \""+command+"\" to "+keyToBind);
	    			KeyBindsMain.addBind(Keyboard.getKeyIndex(keyToBind.toUpperCase()), command);
	    			Minecraft.getMinecraft().thePlayer.sendChatMessage(EnumChatFormatting.GREEN+"Bound '"+command+"' to '"+keyToBind.toUpperCase()+"'.");
	    		}
    		}
    	}
    	if(in.toLowerCase().startsWith("unbind")){
    		if(!in.toLowerCase().contains(" ")){
    			Minecraft.getMinecraft().thePlayer.sendChatMessage(EnumChatFormatting.RED+"Usage: unbind <key>");
    			return;
    		}
    		String[] args = in.toLowerCase().split(" ");
    		if(args.length != 2){
    		...
    		if(args.length > 1){
	    		String keyToBind = args[1];
	    		if(Keyboard.getKeyIndex(keyToBind.toUpperCase()) > 0){
	    			if(KeyBindsMain.keyBound(Keyboard.getKeyIndex(keyToBind.toUpperCase()))){
	    				KeyBindsMod.log("Unbinding "+keyToBind);
	    				KeyBindsMain.removeBind(Keyboard.getKeyIndex(keyToBind.toUpperCase()));
	    				Minecraft.getMinecraft().thePlayer.sendChatMessage(EnumChatFormatting.GREEN+"Unbound '"+keyToBind.toUpperCase()+"'.");
	    			}else{
	        			Minecraft.getMinecraft().thePlayer.sendChatMessage(EnumChatFormatting.RED+"'"+keyToBind.toUpperCase()+"' is not bound to anything!");
	    			}
	...
	public void initGui()
    {
        ...
        this.inputField = new ConsoleTextField(this.fontRendererObj, 4, this.height - 12, this.width - 4, 12);
...
*/
    ...
    public void completePlayerName()
    {
        ...
        if (this.field_73904_o.size() > 1)
        {
            ...
            this.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion( ChatComponentTranslation(var4.toString()), 1);
        }

        this.inputField.writeText((String)this.field_73904_o.get(this.field_73903_n++));
    }

    private IChatComponent ChatComponentTranslation(String string) {
